#!/bin/bash
# corporate-portal-api Lambda deployment script
# Usage: ./deploy.sh [create|update]

set -e

FUNCTION_NAME="corporate-portal-api-dev"
REGION="eu-west-2"
...
echo "=== corporate-portal-api deployment ==="
...
if [ "$1" == "create" ]; then
  ...
  aws lambda create-function \
    ...
    --memory-size 256 \
    --layers $LAYER_ARN \
    --environment "Variables={CORPORATE_TABLE_NAME=durdle-corporate-dev}" \
    --region $REGION
else
