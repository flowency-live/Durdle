 ...
 * @returns {object} Child logger with correlation ID and .log() method
 */
export function createLogger(event, context) {
  ...
  // Add .log() method as alias to .info() for backward compatibility
  // Some Lambdas use logger.log() instead of logger.info()
  childLogger.log = function(eventOrMessage, dataOrMessage) {
    if (typeof eventOrMessage === 'string') {
      ...
      // logger.log({ event: 'foo' }, 'message') -> logger.info({ event: 'foo' }, 'message')
      return childLogger.info(eventOrMessage, dataOrMessage);
    } else {
      // logger.log({ event: 'foo', data: 'bar' }) -> logger.info({ event: 'foo', data: 'bar' })
      return childLogger.info(eventOrMessage);
    }
  };
