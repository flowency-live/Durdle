  ...
  const fetchZoneAssignments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Build assignment map: outwardCode -> zone info
        const assignments: Record<string, ZoneAssignment> = {};
        zones.forEach((zone) => {
          (zone.outwardCodes || []).forEach((code) => {
            assignments[code] = {
              zoneId: zone.zoneId,
              zoneName: zone.name,
            };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <h2 className="text-3xl font-bold text-gray-900 mb-2">Create New Zone</h2>
        <p className="text-gray-600">Define a postcode zone for fixed pricing</p>
      </div>
